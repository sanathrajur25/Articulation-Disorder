...
title('Phoneme articulated by normal child by SLM (Reference)');
y=[y zeros2];
yy=[y zeros3];
dfty=abs(fft(y));
dftyy=abs(fft(yy));
...
for uu=1:length(real_ceps_pitch)
  ...
  pitch_period_To_reference=(16+sample_no)*(1/Fs);
  pitch_freq_FO_reference=1/pitch_period_To_reference;
  xlabel(['Reference pitch frequency: ' num2str(pitch_freq_FO_reference) 'Hz' ]);
